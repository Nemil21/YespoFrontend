...
export default function ScrollCards() {
  ...
  // Intersection Observer for detecting when cards are in view
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry, index) => {
        if (entry.isIntersecting) {
          setActiveCardIndex(index); // Set the active card index when in view
        }
      });
    }, { threshold: 0.5 });
  ...
  return (
    <main className="h-full flex items-center justify-center">
        ...
        <div ref={wrapperRef} className="flex gap-6 w-max py-10 px-6">
          {allCards.map((card: any, index: number) => (
            <div
              ...
              className={`scroll-card w-72 h-96 rounded-2xl flex flex-col items-center justify-center shadow-lg text-xl font-bold p-4 relative overflow-hidden group hover:scale-105 transform transition-all duration-300
                ${activeCardIndex === index ? 'bg-red-500' : ''}`} // Apply red background when active
            >
              <img
                src={card.card_image}
                alt={card.card_title}
      ...
      {/* Text elements with red background when active */}
      <div className="absolute top-5 left-5 space-y-4">
        {['Text 1', 'Text 2', 'Text 3', 'Text 4'].map((text, index) => (
          <div
            key={index}
            className={`text-lg font-bold px-4 py-2 transition-all duration-300 ${activeCardIndex === index ? 'bg-red-500' : 'bg-transparent'}`}
          >
            {text}
          </div>
        ))}
